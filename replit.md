...
- **Collabora is auto-proxied through the app at `${PUBLIC_URL}/collabora`** (see `artifacts/api-server/src/lib/collabora-proxy.ts`). This means operators do NOT need to add a second vhost / proxy host / TLS cert for Collabora — the existing reverse proxy (NPM/Caddy/Traefik/nginx) already routes Collabora because it's the same hostname. The app forwards both HTTP and WS upgrades on `/collabora/*` to the in-container `http://collabora:9980`. Collabora itself runs with `--o:net.proxy_prefix=true` so it generates URLs that respect the `/collabora` prefix. `COLLABORA_URL` defaults to `${PUBLIC_URL}/collabora` in `docker-compose.yml` and only needs overriding if the operator wants Collabora on a separate hostname. The proxy is mounted **before** `express.json`/session middleware in `app.ts` so body-parsing doesn't break the stream, and is anonymous on purpose (document auth lives in the WOPI HMAC access_token in the URL).
